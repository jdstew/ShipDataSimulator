...
public abstract class MessageTools {
    ...
    */
   public static double parseLatitude (char [] latitude) {
...
//                     latitude[i])); //LATITUDE_INCORRECT_CHARACTER
    ...
    */   
   public static char parseLatitudeHemisphere (char [] latHemisphere) {
...
//            "Latitude hemisphere field has incorrect number of characters.")); //LATITUDE_HEMISPHERE_OVERRUN
    ...
    */
   public static double parseLongitude (char [] longitude) {
...
//                     longitude[i]));//LONGITUDE_INCORRECT_CHARACTER
    ...
    * @return parsed value, if successful
    */   
   public static char parseLongitudeHemisphere (char [] longHemisphere) {
...
//            "Longitude hemisphere field has incorrect number of characters."));//LONGITUDE_HEMISPHERE_OVERRUN
    ...
    */ 
   public static long parseUTC (char [] timeUTC) {
      int hours;
      int minutes;
      int seconds;
      int milliseconds = 0;
...
//                     timeUTC[i])); //UTC_INCORRECT_CHARACTER
    ...
    */   
   public static double parseBearing (char [] number) {
      // Test for null length field
      if (number.length == 0) {
         return Double.NaN;
      }
      
      double bearing = parseNumber (number);
...
//            "Check bearing/heading (value is 0.0)."));//HEADING_SUSPICOUS
    ...
    */ 
   public static double parseNumber (char [] number) {
...
//               number[i])); //NUMBER_NON_NUMBER
 ...
 /*
   ...
   */
    ...
    */   
   public static char parseStatus (char [] statusField) {
...
//            "Status field has incorrect number of characters.")); //STATUS_TOO_MANY_CHARACTERS
    ...
    */   
   public static char parseOperatingMode (char [] modeField) {
...
//            "Operating mode field has incorrect number of characters.")); //MODE_TOO_MANY_CHARACTERS
    ...
    */   
   public static double parseLoranGRI (char [] number) {
      // Test for null length field
      if (number.length == 0) {
         return Double.NaN;
      }
...
//               "Invalid number of characters for GRI.")); //LORAN_GRI_INCORRECT
      ...
      double loranGRI = parseNumber (number);
...
//               "GRI is out of range.")); //LORAN_GRI_INCORRECT
    ...
    * @return value of parsed input
    */   
   public static double parseLoranTOATD (char [] number) {
      // Test for null length field
      if (number.length == 0) {
         return Double.NaN;
      }
      
      double loranTD = parseNumber (number);
...
//               "TD is out of range.")); //LORAN_TOATD_INCORRECT
    ...
    * @return value of parsed input
    */   
   public static double parseLoranECDSNR (char [] number) {
      // Test for null length field
      if (number.length == 0) {
         return Double.NaN;
      }
...
//               "Invalid number of characters for ECD or SNR."));//LORAN_ECDSNR_INCORRECT
      ...
      double loranQuality = parseNumber (number);
...
//               "ECD or SNR is out of range."));//LORAN_ECDSNR_INCORRECT
    ...
    */   
   public static char parseLoranStatus (char [] modeField) {
      if (modeField.length == 1) {
         char mode = modeField[0];
         if (mode == 'B') {
//            errorLog.addError (new SentenceError(SentenceErrorTypes.ERR_DATA_INVALID,
